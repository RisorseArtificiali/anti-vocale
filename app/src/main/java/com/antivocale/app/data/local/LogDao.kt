...
interface LogDao {
    ...
    @Query("SELECT id, timestamp, taskId, type, status, prompt, result, errorMessage, durationMs, " +
        ...
        "modelName, rawTranscript, summary, summarySkipReason, failureContext, processingContext FROM logs ORDER BY timestamp DESC LIMIT 500")
    fun getAll(): Flow<List<LogEntity>>

    @Query("SELECT id, timestamp, taskId, type, status, prompt, result, errorMessage, durationMs, " +
        "filePath, audioDurationSeconds, sourcePackageName, isPartial, failedChunkCount, " +
        "modelName, rawTranscript, summary, summarySkipReason, failureContext, processingContext FROM logs WHERE result LIKE '%' || :query || '%' " +
