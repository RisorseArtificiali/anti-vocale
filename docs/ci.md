...
- **Required before the first publish (the one allowed fdroiddata edit per the TASK-465 plan):** change the recipe to `UpdateCheckMode: Tags .*[0-9]$`. The pattern is the text after the space and filters the tag list; the official docs describe exactly this for apps that tag non-release versions (https://f-droid.org/en/docs/Build_Metadata_Reference/#UpdateCheckMode). Alternative without a recipe change: guarantee the nightly commit's versionCode never exceeds the released one (true today: nightly base 41 vs released 4xx codes) and accept that fdroidserver may log an `FDroidException` for the app right after a release while the nightly trails.
