...
- **Release branches MUST be merged back into main the same day the release ships.** A scoped micro-release cut from a side branch (the v1.11.3 pattern: branch off the previous tag, cherry-pick only the fix) leaves main without the version bump, notes, and changelogs: main sat at 1.11.1 while v1.11.2 and v1.11.3 were tagged on parallel lines (found 2026-09-06). After the release is out, `git merge vX.Y.Z` into main, resolving content conflicts toward main (it carries the superset; the release branch is the deliberately reduced scope); the merge's net diff should be version files only (`app/build.gradle.kts`, `docs/play-store/release-notes.xml`, `fastlane/**/changelogs/`). Then bump main's `versionName` to the NEXT planned version with a `-SNAPSHOT` suffix (e.g. `1.12.0-SNAPSHOT`; maintainer decision 2026-09-06) so unreleased builds self-identify in the About row instead of masquerading as the last release; the versionCode stays at the released base until the release bump decides the next one. Proof before cutting any new release: `git log vX.Y.Z --not main` is empty for every shipped tag, and main's versionName is the next-version SNAPSHOT (never equal to the last tag).
