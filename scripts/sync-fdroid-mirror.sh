...
#                        fetches, the fork-parity guard and the diff always
#                        run so the dry run observes the same state the real
#                        run would
...
echo "== refreshing fork (fetch + recipe-parity guard) and mirror (ff-only pull); DRY sees the same state"
...
# The fork branch is NEVER written here: pushing it would start the fdroiddata
# pipeline before the signed reference APKs exist and fail on binary 404s
# (2026-09-01 incident: exactly that, via a manual reconciliation push forced
# by this script's old ff-only pull; removing that pull is the protective
# change, the guard below is early telemetry for finalize's precondition, run
# ~50 min earlier). The fork remote is written only by
# release-fdroid-references.sh finalize, after gate C. A local branch AHEAD of
# origin is the expected pre-finalize state (the recipe commit waiting for its
# push). fdroiddata SQUASH-merges MRs, so commit ancestry cannot express
# "already merged upstream"; the guard is directional on the whole recipe
# minus the machine-managed CurrentVersion fields (regenerated by the
# generator): it fires only on lines origin has that ours lacks (maintainer
# edits anywhere in the file, !47391-style). New local blocks only ever appear
# as our-side changes, and a rebuilt branch fully contains the merged old
# tip, so the normal flow never trips it. diff -u: the default ed-format
# has no + markers.
FORK_BRANCH="$(git -C "$FORK_CHECKOUT" branch --show-current)"
